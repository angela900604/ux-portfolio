      ...
      {items.map((item, i) => (
        <li key={item.slug} className="h-full min-h-0">
          <FadeInSection className="h-full" delay={0.05 + i * 0.05}>
            <div className="group flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-ink-line bg-paper transition duration-300 hover:-translate-y-1 hover:border-ink hover:shadow-[0_20px_48px_-28px_rgba(0,0,0,0.12)]">
              <div className="relative aspect-[16/11] overflow-hidden bg-paper-soft">
                <Link
                  href={item.href}
                  className="absolute inset-0 block rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-ink/20 focus-visible:ring-offset-2 focus-visible:ring-offset-paper"
                  aria-label={`Open case study: ${item.title}`}
                >
                  <CoverMedia item={item} />
                </Link>
                <div
                  className="pointer-events-none absolute inset-0 bg-gradient-to-t from-paper/50 to-transparent opacity-0 transition duration-300 group-hover:opacity-100"
                  aria-hidden
                />
              </div>
              <Link
              ...
              >
                ...
                <h3 className="mt-3 font-sans text-xl font-medium tracking-tight text-ink group-hover:text-ink sm:text-2xl">
                  {item.title}
                </h3>
                <p className="mt-2 text-xs text-ink-faint">{item.year}</p>
              </Link>
            </div>
          </FadeInSection>
        </li>
