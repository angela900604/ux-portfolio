      ...
      >
        ...
        <div className="pointer-events-none absolute inset-y-0 left-0 right-0 flex items-center justify-between px-2">
          <button
            type="button"
            onClick={() => go(-1)}
            className="pointer-events-auto rounded-full border border-zinc-600/80 bg-zinc-950/85 px-3 py-2 text-sm text-zinc-100 shadow-lg backdrop-blur-sm transition hover:bg-zinc-800 hover:text-white focus:outline-none focus:ring-2 focus:ring-[#F20C90]/80"
        ...
        {items.map((_, i) => (
          <button
            ...
            className={`h-2 w-2 rounded-full transition ${
              i === safeIndex
                ? "bg-[#F20C90]"
                : "bg-zinc-600 hover:bg-[#004291]/60"
            }`}
